        ...
        <div className="w-full md:w-1/2 flex flex-col">
          ...
          <div className="w-full flex justify-center">
            ...
            >
              {/* Category Cards */}
              {[
                { title: "Cold Drinks", count: 49, icon: "🥤", bg: "bg-purple-100" },
                { title: "Bakery", count: 8, icon: "🥐", bg: "bg-yellow-100" },
                { title: "Vegetables", count: 485, icon: "🥕", bg: "bg-red-100" },
                { title: "Fruits", count: 291, icon: "🍒", bg: "bg-green-100" }
              ].map((category, index) => (
                <SwiperSlide key={index}>
                  <div className={`p-6 ${category.bg} rounded-lg shadow-md flex flex-col items-center`}>
                    <span className="text-3xl">{category.icon}</span>
                    <p className="text-lg font-semibold mt-2">{category.title}</p>
                    <span className="text-gray-600 text-sm">{category.count} items</span>
                  </div>
                </SwiperSlide>
              ))}
            </Swiper>
